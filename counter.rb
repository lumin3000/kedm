...
def invitation_st
  ...
  group(arr).each do |key, value|
    $hash[key] && ($hash[key][:inv_num] = value)
  end
end
...
def register_st
  ...
  h = group(arr)
  h.each do |key, value|
    $hash[key] && ($hash[key][:reg_num] = value)
  end
end

def percentage(a, b)
  return "-" if b.to_i == 0
  ((a.to_f/b.to_f*10000).to_i.to_f/100).to_s+"%"
end
...
def st
  ...
  $hash.each do |key, value|
    reg_num += value[:reg_num].to_i
    inv_num += value[:inv_num].to_i
    puts "#{value[:uri]}(#{value[:comment]}) : #{value[:reg_num] || 0}/#{value[:inv_num] || 0} " + percentage(value[:reg_num], value[:inv_num])
  end
  puts "                                   -------------------"
  puts "                                   共 : #{reg_num}/#{inv_num} #{percentage(reg_num, inv_num)}"
end
...
$hash = {
  ...
  # "jbcdg9joomh86800002" => {uri: "leilak", comment: "leilak                       "},
}
puts "------ #{$date}统计 ------"
